...
//Edit Post Route
app.put("/posts/:id", isLoggedIn, isPostOwner, upload.single('image'), validatePost, wrapAsync(async (req, res) => {
  const { id } = req.params;
...
//Like Route
app.post("/posts/:id/likes", isLoggedIn, wrapAsync(async (req, res) => {
  const { id } = req.params;
